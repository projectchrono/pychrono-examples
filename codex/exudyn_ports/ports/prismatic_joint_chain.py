...
# Reproduces the intent of EXUDYN Examples/addPrismaticJoint.py:
# a four-link rigid-body chain under gravity with prismatic joints whose axes
# are specified in the global frame.
...
def make_link(index, center, rotation):
    ...
    body.GetVisualShape(0).SetColor(color(0.82, 0.18 + 0.08 * index, 0.12))
    return body
...
def run_visual(duration, step):
    ...
    vis.SetWindowTitle("EXUDYN port: addPrismaticJoint.py")
    vis.Initialize()
    vis.AddSkyBox()
...
def main():
    ...
    print("EXUDYN port: addPrismaticJoint.py -> PyChrono prismatic joint chain")
    if args.no_vis:
