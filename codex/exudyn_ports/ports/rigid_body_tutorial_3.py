...
# Reproduces the intent of EXUDYN Examples/rigidBodyTutorial3.py:
# two 3D rigid bodies created with helper functions, revolute joints at the
# ground and elbow, plus a global force and torque applied to the second link.
...
def build_system():
    ...
    link0.SetName("tutorial3 first link")
    ...
    link1.SetName("tutorial3 second link")
    ...
    load_container.Add(
        chrono.ChLoadBodyForce(
            link1,
            chrono.ChVector3d(0, FORCE_Y, 0),
            False,
            chrono.ChVector3d(0, 0, 0.5 * LENGTH),
            True,
        )
    )
    load_container.Add(chrono.ChLoadBodyTorque(link1, chrono.ChVector3d(TORQUE_X, 0, 0), False))
...
def run_visual(duration, step):
    ...
    vis.SetWindowTitle("EXUDYN port: rigidBodyTutorial3.py")
    vis.Initialize()
    vis.AddSkyBox()
...
def print_state(system, links):
    ...
    print(
        ...
        f"force_y={FORCE_Y:+.3f}  torque_x={TORQUE_X:+.3f}"
    )
...
def main():
    ...
    print("EXUDYN port: rigidBodyTutorial3.py -> PyChrono two-link tutorial with loads")
    if args.no_vis:
