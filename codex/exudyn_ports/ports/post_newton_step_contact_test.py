...
# Reproduces the intent of EXUDYN TestModels/postNewtonStepContactTest.py:
# a one-mass penalty contact problem where a sphere falls onto a unilateral
# stop. EXUDYN resolves switching through a post-Newton step-size callback; this
# PyChrono port uses native SMC sphere-floor contact for the same contact event.
...
def build_system():
    ...
    floor = chrono.ChBodyEasyBox(0.8, 0.018, 0.22, 1000, True, True, material)
    floor.SetName("post-Newton contact floor")
    floor.SetFixed(True)
    ...
    sphere.SetName("post-Newton contact mass sphere")
    sphere.SetMass(MASS)
    sphere.SetPos(chrono.ChVector3d(0, INITIAL_Y, 0))
    sphere.SetPosDt(chrono.ChVector3d(0, INITIAL_VY, 0))
    sphere.GetVisualShape(0).SetColor(color(0.88, 0.14, 0.10))
    system.AddBody(sphere)

    guide = chrono.ChBodyEasyBox(0.012, 0.45, 0.012, 1000, True, False)
    guide.SetName("visible post-Newton contact height guide")
    guide.SetFixed(True)
    guide.SetPos(chrono.ChVector3d(-0.18, 0.16, 0))
...
def run_visual(duration, step):
    ...
    vis.SetWindowTitle("EXUDYN port: postNewtonStepContactTest.py")
    vis.Initialize()
    vis.AddSkyBox()
    vis.AddCamera(chrono.ChVector3d(0.45, 0.55, 0.50), chrono.ChVector3d(0, 0.08, 0))
...
def main():
    ...
    print("EXUDYN port: postNewtonStepContactTest.py -> PyChrono SMC contact switch")
    if args.no_vis:
