...
# Reproduces the intent of EXUDYN Examples/springDamperTutorialNew.py:
# same tutorial oscillator as springDamperTutorial.py, expressed in EXUDYN with
# create functions: a mass point with initial x displacement/velocity, spring
# damper to ground, and a constant x load.
...
def build_system():
    ...
    mass.GetVisualShape(0).SetColor(color(0.10, 0.36, 0.90))
    system.AddBody(mass)
...
def run_visual(duration, step):
    ...
    vis.SetWindowTitle("EXUDYN port: springDamperTutorialNew.py")
    vis.Initialize()
    vis.AddSkyBox()
...
def main():
    ...
    print("EXUDYN port: springDamperTutorialNew.py -> PyChrono TSDA tutorial oscillator")
    if args.no_vis:
