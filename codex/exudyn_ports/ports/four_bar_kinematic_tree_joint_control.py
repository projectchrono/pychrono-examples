...
# Reproduces EXUDYN Examples/FurtherExamples/fourBarKinematicTreeJointControl.py:
# a three-link kinematic-tree four-bar with a closing-loop GenericJoint and
# built-in joint velocity control on the first RevoluteZ coordinate.  Chrono
# represents the same geometry as explicit visible rigid links with revolute
# loop joints; the EXUDYN jointVelocityOffsetVector/jointDControlVector pair is
# reproduced by updating the base revolute actuator torque each step.
...
def make_axis_marker(system, name, point, radius=0.030):
    marker = chrono.ChBodyEasyCylinder(chrono.ChAxis_Z, radius, 0.30, 1000, True, False)
    marker.SetName(name)
...
def make_control_indicator(system, point):
    indicator = chrono.ChBody()
    indicator.SetName("kinematic-tree joint D-control indicator")
    indicator.SetFixed(True)
    indicator.EnableCollision(False)

    hub = chrono.ChVisualShapeCylinder(0.040, 0.32)
    hub.SetColor(color(0.92, 0.22, 0.12))
    indicator.AddVisualShape(hub, chrono.ChFramed(point + chrono.ChVector3d(0, 0, 0.03), chrono.QUNIT))

    arrow = chrono.ChVisualShapeBox(0.16, 0.026, 0.026)
    arrow.SetColor(color(0.92, 0.22, 0.12))
    indicator.AddVisualShape(arrow, chrono.ChFramed(point + chrono.ChVector3d(0.11, 0.08, 0.16), chrono.QuatFromAngleZ(0.45)))

    tip = chrono.ChVisualShapeCone(0.050, 0.08)
    tip.SetColor(color(0.92, 0.22, 0.12))
    indicator.AddVisualShape(tip, chrono.ChFramed(point + chrono.ChVector3d(0.19, 0.13, 0.16), chrono.QuatFromAngleZ(0.45)))
...
def build_system():
    ...
    link1 = make_link(system, "joint-control kinematic-tree crank", p0, p1, L1, color(0.92, 0.22, 0.12))
    link2 = make_link(system, "joint-control kinematic-tree coupler", p1, p2, L2, color(0.12, 0.42, 0.85))
    link3 = make_link(system, "joint-control kinematic-tree rocker", p2, p3, L3, color(0.10, 0.62, 0.28))

    base_joint = add_revolute(system, "D-controlled base revolute Z", link1, ground, p0)
    ...
    for index, point in enumerate(points):
        make_axis_marker(system, f"visible joint-control pin axis {index + 1}", point)
    indicator = make_control_indicator(system, p0)
    ...
    system._four_bar_joint_control_items = items
    update_joint_control(items)
    return system, items


def update_joint_control(items):
    link1 = items["links"][0]
    omega_z = link1.GetAngVelParent().z
    torque_z = D_CONTROL * (TARGET_OMEGA_Z - omega_z)
...
def update_visuals(system):
    update_joint_control(system._four_bar_joint_control_items)
...
def run_visual(duration, step):
    ...
    vis.SetWindowTitle("EXUDYN port: fourBarKinematicTreeJointControl.py")
    vis.Initialize()
    vis.AddSkyBox()
...
def print_state(system, items):
    ...
    print(
        ...
        f"control_torque={items['last_torque']:+.6f}  "
...
def main():
    ...
    print("EXUDYN port: fourBarKinematicTreeJointControl.py -> PyChrono explicit D-controlled four-bar")
    if args.no_vis:
