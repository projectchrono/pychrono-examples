...
# Reproduces the intent of EXUDYN Examples/rigidBodyTutorial2.py:
# two 3D rigid links under gravity, connected by generic joints equivalent to
# revolute supports with different free axes.
...
def build_system():
    ...
    link0.SetName("first rigid tutorial link")
    ...
    link1.SetName("second rigid tutorial link")
    link1.SetPos(chrono.ChVector3d(LENGTH, 0, 0.5 * LENGTH))
    link1.GetVisualShape(0).SetColor(color(0.12, 0.42, 0.85))
    ...
    for point, tint in [
        (chrono.ChVector3d(0, 0, 0), color(0.08, 0.08, 0.08)),
        (chrono.ChVector3d(LENGTH, 0, 0), color(0.08, 0.08, 0.08)),
    ]:
...
def run_visual(duration, step):
    ...
    vis.SetWindowTitle("EXUDYN port: rigidBodyTutorial2.py")
    vis.Initialize()
    vis.AddSkyBox()
...
def main():
    ...
    print("EXUDYN port: rigidBodyTutorial2.py -> PyChrono two-link rigid tutorial")
    if args.no_vis:
