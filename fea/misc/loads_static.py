...
# As before, create a custom load with stiff force, acting on MULTIPLE nodes
# at once. This time we will need the ChLoadCustomMultiple as base class.
# Those nodes (i.e. ChLoadable objects) can be added in mesh in whatever
# order, not necessarily contiguous, because the bookkeeping is automated.
# Being a stiff load, a jacobian will be automatically generated by default
# using numerical differentiation but if you want you can override
# ComputeJacobian() and compute mK, mR analytically - see prev. example.
